...
using Mall3s.Core.Message.Service;
using Mall3s.Core.System.Manager;
using Mall3s.Core.System.Service;
using Mall3s.Core.VisualDev.Service;
...
using Mall3s.WorkFlow.Entitys.Dto.FlowBefore;
using Mall3s.WorkFlow.Entitys.Dto.FlowTask;
using Mall3s.WorkFlow.Entitys.Enum;
using Mall3s.WorkFlow.Entitys.Model;
using Mall3s.WorkFlow.Entitys.Model.Properties;
using Mapster;
...
namespace Mall3s.Core.WorkFlow.Service.Impl
{
    public class FlowTaskCoreService
    {
        ...
        private readonly IFlowDelegateCoreService _flowDelegateService;
        private readonly IFlowEngineCoreService _flowEngineService;
        ...
        private readonly IMessageCoreService _messageService;
        private readonly IUserRelationCoreService _userRelationService;
        private readonly IUserCoreManager _userManager;
        private readonly IRunCoreService _runService;
        private readonly IBillRullCoreService _billRullService;
        private readonly IVisualDevCoreService _visualDevServce;
        ...
        private readonly ISynThirdInfoCoreService _synThirdInfoService;
        private readonly ILogger<FlowTaskCoreService> _logger;
        public FlowTaskCoreService(IFlowTaskCoreRepository flowTaskRepository,
            IDictionaryDataCoreService dictionaryDataService, IUsersCoreService usersService,
            IFlowEngineCoreService flowEngineService, IOrganizeCoreService organizeService,
            IDataInterfaceCoreService dataInterfaceService, IFlowDelegateCoreService flowDelegateService,
            IMessageCoreService messageService, IUserRelationCoreService userRelationService,
            IUserCoreManager userManager, IRunCoreService runService, IBillRullCoreService billRullService,
            IVisualDevCoreService visualDevServce, IDataBaseService dataBaseService,
            IDbLinkService dbLinkService, ISysConfigCoreService sysConfigService,
            ISynThirdInfoService synThirdInfoService, ILogger<FlowTaskCoreService> logger)
        {
        ...
        public async Task Reject(FlowTaskEntity flowTaskEntity, FlowTaskOperatorEntity flowTaskOperatorEntity, FlowHandleModel flowHandleModel, int formType)
        {
            try
            {
                ...
                await CreateNextFlowTaskOperator(flowTaskNodeEntityList, flowTaskNodeEntity, approversProperties,
                    ...
                    flowTaskOperatorEntityList, formData);
                #endregion
        ...
        public async Task Recall(string id, FlowHandleModel flowHandleModel)
        {
            try
            {
                ...
                if (flowTaskOperatorEntity.HandleStatus == 0)
                    throw Mall3sException.Oh(ErrorCode.WF0010);
                //任务待审状态才能撤回
                if (!(flowTaskEntity.EnabledMark == 1 && flowTaskEntity.Status == 1))
                    throw Mall3sException.Oh(ErrorCode.WF0011);
                //撤回节点下一节点已操作
            ...
            {
                DbScoped.Sugar.RollbackTran();
                throw Mall3sException.Oh(ErrorCode.WF0005);
            }
        }
        ...
        public async Task Assigned(string id, FlowHandleModel flowHandleModel)
        {
            try
            {
                ...
                if (isOk < 1)
                    throw Mall3sException.Oh(ErrorCode.WF0008);
        ...
        public async Task Transfer(string id, FlowHandleModel flowHandleModel)
        {
            try
            {
                ...
                if (flowOperatorEntity == null)
                    throw Mall3sException.Oh(ErrorCode.COM1005);
                ...
                if (isOk < 1)
                    throw Mall3sException.Oh(ErrorCode.WF0007);
        ...
        private async Task AddFlowTaskOperatorEntityByAssigneeType(List<FlowTaskOperatorEntity> flowTaskOperatorEntityList, List<FlowTaskNodeEntity> flowTaskNodeEntitieList, FlowTaskNodeEntity flowTaskNodeEntity, FlowTaskNodeEntity nextFlowTaskNodeEntity, string creatorUserId, string fromData, int type = 1)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw Mall3sException.Oh(ErrorCode.WF0013);
            }
        }
        ...
        private async Task CreateNextFlowTaskOperator(List<FlowTaskNodeEntity> flowTaskNodeEntityList, FlowTaskNodeEntity flowTaskNodeEntity,
            ...
            List<FlowTaskOperatorEntity> nextFlowTaskOperatorEntityList, string fromData)
        {
            try
            {
