    ...
    public class SucceededUnifyResultFilter : IAsyncActionFilter, IOrderedFilter
    {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            else
            {
                ...
                if(!actionExecutedContext.HttpContext.Request.Path.HasValue || 
                    (actionExecutedContext.HttpContext.Request.Path.HasValue && !actionExecutedContext.HttpContext.Request.Path.Value.EndsWith("/v2/api-docs")))
                { 
