...
using Mall3s.Templates.Extensions;
using System;
...
namespace Mall3s.TaskScheduler
{
    ...
    public static class SpareTime
    {
        ...
        public static void Do(Func<double> intervalHandler, Func<SpareTimer, long, Task> doWhat = default, string workerName = default, string description = default, bool startNow = true, bool cancelInNoneNextTime = true, SpareTimeExecuteTypes executeType = SpareTimeExecuteTypes.Parallel, bool continued = true, bool onlyInspect = false)
        {
            ...
            Func<object, ElapsedEventArgs, Task> handler = async (sender, e) =>
            {
                ...
                if (Interlocked.Exchange(ref interlocked, 1) == 0)
                {
                    ...
                    {
                        // 执行异常通知
                        if (timer.Type == SpareTimeTypes.Interval && !onlyInspect) await WriteChannel(timer, 3, ex.Message);
        ...
        /// 判断任务是否存在
        /// </summary>
        public static bool AnyTask(string workerName)
        {
            if (!WorkerRecords.Any()) return false;

            return WorkerRecords.TryRemove(workerName, out var workerRecord);
        }
        ...
        /// <returns></returns>
        private static async Task WriteChannel(SpareTimer timer, int statues, string msg = "")
        {
            await ChannelContext<SpareTimerExecuter, SpareTimeListenerChannelHandler>.BoundedChannel.Writer.WriteAsync(new SpareTimerExecuter(timer, statues, msg));
        }
